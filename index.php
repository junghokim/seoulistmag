...
//	$assign_to_config['site_404'] = '';
	$assign_to_config['global_vars'] = array(
		'snip_all_articles'		=> '="open|Featured Article|Front Page|Item of Interest"',
		'snip_all_channels'		=> '="articles|columns|issue"',
		'snip_all_permalink_switch' => '
		{if channel_id=="1"}
			<a href="{title_permalink="articles/read"}">{title}</a>
			{if:elseif channel_id=="4"}
			<a href="{title_permalink="guides/read"}">{title}</a>
			{if:elseif channel_id=="3"}
			<a href="{title_permalink="columns/read"}">{title}</a>
			{if:elseif channel_id=="2"}
			<a href="{title_permalink="issue/index"}">{title}</a>
			{if:else}
		{/if}
		',
		'snip_all_permalink_switch_opening' =>'
		{if channel_id=="1"}
			<a href="{title_permalink="articles/read"}">
			{if:elseif channel_id=="4"}
			<a href="{title_permalink="guides/read"}">
			{if:elseif channel_id=="3"}
			<a href="{title_permalink="columns/read"}">
			{if:elseif channel_id=="2"}
			<a href="{title_permalink="issue/index"}">
			{if:else}
		{/if}
		',
		'snip_date_short'		=> 'format="%Y %M %j"',
		'snip_date_long'		=> 'format="%Y %F %j"',
		'categories_top_level'	=> 'show="1|16|11|19|6|5|20|21|22|25|26|27|28" limit="1"',
		'categories_secondary_level'	=> 'show="1|2|3|4|17|18|12|13|14|15|7|8|9|10|23|24|28" limit="1"',
		  // Added 1 and 28 for parent categories with no children so that they are shown in section listings
		'snip_disqus_comment-count'		=> '
		...
		{if channel_id=="1"}
			...
			{if:else}
		{/if}		
		#disqus_thread" data-disqus-identifier="{url_title}"></a></span>'
