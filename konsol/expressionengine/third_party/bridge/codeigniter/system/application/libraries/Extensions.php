 ...
 *
 * This file is copyright EllisLab, Inc. and is distributed with permission.
 *
 * @package		ExpressionEngine
 * @subpackage	Core
 * @category	Core
		...
		foreach($this->extensions[$which] as $priority => $calls)
		{
			foreach($calls as $class => $metadata)
			{
				...
				if ( ! file_exists($path))
				{
					...
					{
						$name = $this->EE->security->sanitize_filename(strtolower(substr($class, 0, -10))); // remove '_extension' suffix
					}
